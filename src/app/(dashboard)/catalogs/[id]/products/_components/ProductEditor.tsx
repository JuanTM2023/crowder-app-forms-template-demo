...
function toDraftVariant(v: ProductVariant): DraftVariant {
  return {
    ...
    serviceFee: v.serviceFee != null ? String(v.serviceFee) : "",
...
export function ProductEditor({
  ...
  const [currency, setCurrency] = useState(
    product?.currency ?? catalogCurrency ?? "",
  );
  ...
  const buildInput = (): ProductInput => ({
    ...
    variants: variants.map((v) => ({
      ...
      serviceFee: v.serviceFee.trim() ? Number(v.serviceFee) : 0,
      ...
      {tab === "variants" && (
        <div className="space-y-4">
          ...
          <div className="space-y-1 rounded-md bg-subtle/50 p-3 text-xs text-muted-foreground">
            ...
            <p>
              Cada variante es una versión vendible del producto: una
              combinación concreta de talla, color u otra opción. Si el producto
              no tiene variaciones, dejá una sola variante con su precio.
            </p>
            <ul className="list-disc space-y-0.5 pl-4">
              <li>
                <span className="text-foreground">Título</span>: nombre visible
                de la combinación (ej: <em>M / Rojo</em>).
              </li>
              <li>
                <span className="text-foreground">Opciones</span>: pares
                <em> clave=valor</em> separados por coma (ej:{" "}
                <em>Talla=M, Color=Rojo</em>). Es lo que distingue una variante
                de otra.
              </li>
              <li>
                <span className="text-foreground">SKU</span>: código interno
                único; opcional pero recomendado para conciliar inventario.
              </li>
              <li>
                <span className="text-foreground">Precio</span>: puede variar
                por variante (ej: tallas grandes más caras).
              </li>
              <li>
              ...
              <Table>
                <TableHead>
                  <TableRow>
                    ...
                    <TableHeaderCell className="min-w-28">
                      Precio
                    </TableHeaderCell>
                    <TableHeaderCell className="min-w-28">
                      Servicio
                    </TableHeaderCell>
                    <TableHeaderCell className="min-w-56">
                      Stock
                    </TableHeaderCell>
                    <TableHeaderCell className="min-w-48">
                      Imágenes
                    ...
                    <TableRow key={i} className="align-top">
                      <TableCell>
                        <Input
                          value={v.title}
                          onChange={(e) =>
                            setVariant(i, { title: e.target.value })
                          }
                          placeholder="M / Rojo"
                          disabled={readOnly}
                        />
                      ...
                      <TableCell>
                        <Input
                          value={v.optionsText}
                          onChange={(e) =>
                            setVariant(i, { optionsText: e.target.value })
                          }
                        ...
                        <div className="space-y-2">
                          <label className="flex items-center gap-2 text-sm">
                            <Checkbox
                              ...
                              onCheckedChange={(c) =>
                                setVariant(i, { stockTracked: c === true })
                              }
                              disabled={readOnly}
                            />
