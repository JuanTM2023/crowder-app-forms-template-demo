...
export function EmbedWizard(props: Props) {
  ...
  async function handleParentSubmit(p: Extract<Phase, { kind: "ready" }>) {
    if (submitting) return;
    setSubmitError(null);
    setServerErrors([]);
  ...
  return (
    ...
    >
      ...
      {phase.kind === "fatal" && (
        <PhaseStatus title={phase.title} detail={phase.message} tone="error" />
      )}
      ...
      {phase.kind === "ready" && currentStep && (
        <WizardBody
          ...
          onPreviewSubmit={
            previewMode
              ? () =>
                  void handleParentSubmit(
                    phase as Extract<Phase, { kind: "ready" }>,
                  )
              : undefined
          }
        />
  ...
  if (productLists) {
    ...
    for (const [key, list] of Object.entries(productLists)) {
      if (key.startsWith(prefix))
        productsByQuestion[key.slice(prefix.length)] = list;
    }
  }
  return (
...
function formatValue(v: unknown): string {
  ...
  if (Array.isArray(v)) {
    return v
      .map((item) =>
        isProductPick(item) ? formatProductPick(item) : String(item),
      )
      .join(", ");
  }
