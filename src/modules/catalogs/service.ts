...
export async function updateCollection(
  ...
  if (!next)
    throw new DomainError("not_found", `colección '${id}' no encontrada`);
  return next;
}
...
function normalizeManualVariants(
  ...
  const derived =
    options && options.length > 0 ? options : deriveOptions(normalized);
  return {
...
export async function updateProduct(
  ...
  if (patch.serviceFee !== undefined) set.serviceFee = patch.serviceFee;
  if (patch.collectionIds !== undefined)
    set.collectionIds = patch.collectionIds;
