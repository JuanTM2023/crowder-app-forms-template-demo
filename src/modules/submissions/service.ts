...
export async function editSubmission(input: {
  ...
  if (!current) {
    throw new DomainError(
      "not_found",
      `submission '${input.submissionId}' not found`,
    );
  }
  ...
  if (!parsed.success) {
    const errors: ValidationError[] = parsed.error.issues.map((issue) => ({
      ...
      questionId: typeof issue.path[0] === "string" ? issue.path[0] : undefined,
      code: issue.code,
      message: issue.message,
    }));
    throw new DomainError("invalid_payload", "Hay respuestas inválidas", {
      errors,
    });
  }
...
async function deriveForValidated(
  ...
  const listingConfigs = new Map<
    string,
    NonNullable<FormQuestion["product"]>
  >();
  for (const v of validated) {
